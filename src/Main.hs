...
main = do
  ...
  if not (showHelp opts)
   then do let state = transformOptions opts
           ...
           case outputDir opts of
             Just out -> do writeFile out (pPrint m)
                            printDebug (enableDebug opts) state
             Nothing  -> do putStr $ pPrint m
                            printDebug (enableDebug opts) state
...
printDebug b s | b         = print $ "DebugOutput:" ++ debugOutput s -- TODO pretty debug
               | otherwise = return ()

pPrint :: Module () -> String
pPrint = prettyPrintStyleMode (Style {mode = PageMode, lineLength = 120, ribbonsPerLine = 1.5}) (defaultMode)
