...
rhsToIf
  :: Rhs ()      -- the (maybe guarded) righthandside
  -> Exp ()      -- next case
...
extract (GuardedRhs _ [s] e) = applyGEExp e
...
applyGEModule (Module _ mmh mps ids ds) = do
...
comp (Match _ name _ _ _) (Match _ name2 _ _ _) =
  selectNameStr name == selectNameStr name2
comp (InfixMatch _ _ name _ _ _) (InfixMatch _ _ name2 _ _ _) =
  selectNameStr name == selectNameStr name2
comp _ _ = False
