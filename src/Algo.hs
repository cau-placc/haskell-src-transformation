...
match :: [Pat ()]             -- list of variables (variable Pattern) fresh vars
...
match vars@(x:xs) eqs      er
  | allVars eqs = do eqs' <- (mapM (substVars x) eqs )
                     match xs eqs' er  -- Rule 1
  | allCons eqs = makeRhs x xs eqs er                   -- Rule 2
...
substVars pv ((p:ps),e) = do s1 <- getPVarName p
                             s2 <- getPVarName pv
                             let sub = subst s1 s2
...
getPVarName (PWildCard _)   = do n <- freshVar
                                 return $ 'a' : show n
...
groupPat = groupBy ordPats
 where
  ordPats (x,_) (y,_) = isPVar (head x) && (isPVar (head y))
...
createRekMatch :: [Pat ()]                   -- list of variables to match
               -> Exp ()                     -- error
               -> [[Eqs]]                    -- alternating groups
               -> PM (Exp ())                -- result
createRekMatch vars er = foldr (\eqs mrhs -> mrhs >>= match vars eqs) (return er)
...
makeRhs :: Pat ()               -- fresh variable for case expression
        -> [Pat ()]             -- list of fresh variables which are to match
        -> [Eqs]                -- list of tuples of pattern and right sides
        -> Exp ()               -- error
        -> PM (Exp ())          -- resulting right hand side
makeRhs x xs eqs er = do alts <- computeAlts x xs eqs er
...
computeAlts x xs eqs er = do
  ...
  case mxs of
      [] -> return alts
      zs -> do b <- gets trivialCC
               if b then return $ alts ++ [B.alt B.wildcard err]
                    else do z <- createAltsFromConstr x zs er
...
findCons :: [Constructor]                                                       -- TODO rename // complement
         -> [QName ()]
         -> [Constructor]
findCons cons usedcons = filter ( \con -> not(elem (getConstrName con) usedcons)) cons
...
findDataType :: QName ()                   -- Constructor name
             -> [(String, [Constructor])]  -- [(Datatype, [(Konstruktor,Arität)])]
             -> (String,[Constructor])     -- Datatype
findDataType cname
  = foldr ( \c sc -> if elem cname (map getConstrName (snd c)) then c else sc) (error "no data Type") -- todo fst und snd als synonym
...
createAltsFromConstr :: Pat ()        -- variable from case exp
...
computeAlt :: Pat ()               -- variable from case exp
...
translateApp (PApp _ qn ps)         = foldl (\acc x -> (App () acc (translatePVar x))) (Con () qn) ps
...
getConst (PApp _ qname ps)         = do nvars <- newVars (length ps)
                                        return ((PApp () qname nvars) , nvars, ps)
getConst (PInfixApp _ p1 qname p2) = do nvars <- newVars 2
                                        let [nv1,nv2] = nvars
                                            ps        = [p1,p2]
                                        return ((PInfixApp () nv1 qname nv2), nvars,ps)
getConst (PParen _ p)              = getConst p
getConst (PList _ ps)              | length ps >= 1 = do let (n:nv) = ps
                                                             listCon = Special () $ Cons ()
                                                         getConst (PInfixApp () n listCon (PList () nv))
                                   | length ps == 0 = return ((PList () []), [] , [])
getConst (PTuple _ bxd ps)         = do nvars <- newVars (length ps)
...
optimize ex = case ex of
     InfixApp _ e1 qop e2  -> do e1' <- optimize e1
                                 e2' <- optimize e2
                                 return $ InfixApp () e1' qop e2'
     App _ e1 e2           -> do e1' <- optimize e1
                                 e2' <- optimize e2
                                 return $ App () e1' e2'
     Lambda _ ps e         -> do e' <- optimize e
                                 return $ Lambda () ps e'
     Let _ b e             -> do e' <- optimize e
                                 return $ Let () b e'
     If _ e1 e2 e3         -> do e1' <- optimize e1
                                 e2' <- optimize e2
                                 e3' <- optimize e3
                                 return $ If () e1' e2' e3'
     Case _ e alts         -> optimizeCase e alts
     Do _ _                -> error "optimize : do is not supported"
     Tuple _ bxd es        -> do es' <- mapM optimize es
                                 return $ Tuple () bxd es'
     List _ es             -> do es' <- mapM optimize es
                                 return $ List () es'
     Paren _ e             -> do e' <- optimize e
                                 return $ Paren () e'
     c                     -> return c
...
optimizeCase e alts | isVarExp e   = do mpats <- gets matchedPat
                                        case lookup e mpats of                  -- lookupBy ?
                                           Just pat ->  renameAndOpt pat alts  -- look for the correct pattern replace, case exp and rename
                                           Nothing  ->  addAndOpt  e alts      -- stackwise add it to first place and then remove first
                    | otherwise = do e'    <- optimize e
...
renameAndOpt :: Pat ()      -- found Pattern for variable
             -> [Alt ()]    -- alternatives from the case exp
             -> PM (Exp ())
renameAndOpt pat alts = let aPaR = map (\ (Alt _ p r _) -> (p,r)) alts
                            patQ = getQNamePat pat
                            sameCons = filter (\ (p,_) -> cheatEq (getQNamePat p) patQ ) aPaR
                        in case sameCons of
                            [] -> error $ "Found in case but not found in alts : Tried" ++ (show patQ) ++ " Searched in " ++ (show (map fst aPaR))
                            ((p,r):_) -> do let e  =  selectExp r
                                                p1 = selectPats pat
                                                p2 = selectPats p
...
renameAll ((from,to):r) e = do f <- getPVarName from
                               t <- getPVarName to
                               res <- renameAll r e
...
addAndOpt e alts = do alts' <- mapM (bindAndOpt e) alts
                      return $ Case () e alts'
 where
  bindAndOpt :: Exp () -> Alt () -> PM (Alt ()) -- uses the list of Exp Pat as a stack
  bindAndOpt v a@(Alt _ p _ _) = do stack <- gets matchedPat
...
optimizeAlt (Alt _ p rhs _ ) = do let (UnGuardedRhs _ e) = rhs
                                  e' <- optimize e
