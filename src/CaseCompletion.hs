module CaseCompletion ( completeCase
                      , applyCCModule) where
...
completeCase insideLet (Case _ expr as) = do
    ...
    eqs' <- mapM (\ (p, ex)  -> completeCase insideLet ex >>= \e -> return (p, e)) eqs
    res <- match [v] eqs' err
...
completeCase il (InfixApp _ e1 qop e2) = do exp1 <- completeCase il e1
                                            exp2 <- completeCase il e2
                                            return $ InfixApp () exp1 qop exp2
completeCase il (App _ e1 e2)          = do exp1 <- completeCase il e1
                                            exp2 <- completeCase il e2
                                            return $ App () exp1 exp2
completeCase il (Lambda _ ps e)        = completeLambda ps e il
completeCase il (Let _ binds e)        = do e'     <- completeCase il e -- undefined -- TODO
                                            binds' <- completeBindRhs binds
                                            return $ Let () binds' e'
completeCase il (If _ e1 e2 e3)        = do exp1 <- completeCase il e1
                                            exp2 <- completeCase il e2
                                            exp3 <- completeCase il e3
                                            return $ If () exp1 exp2 exp3
completeCase il (Tuple _ boxed es)     = do exps <- mapM (completeCase il) es  -- complete case für List Expression
                                            return $ Tuple () boxed exps
completeCase il (Paren _ e1)           = do exp1 <- completeCase il e1
                                            return $ Paren () exp1
...
completeBindRhs (BDecls _ dcls) = do dcls' <-  mapM (applyCCDecl True) dcls
                                     return $ BDecls () dcls'
...
applyCCModule (Module _ mmh mps ids ds) = do dcls <- mapM (applyCCDecl False) ds
                                             return $ Module () mmh mps ids dcls
applyCCModule _                         = error "applyCCModule: not on module"

applyCCDecl :: Bool -> Decl () -> PM (Decl ())
applyCCDecl insideLet (FunBind _ ms )   = do nms <- applyCCMatches insideLet ms
                                             return (FunBind () nms)
applyCCDecl insideLet (PatBind _ p r _) = if isPVar p
                                           then do let e = (\(UnGuardedRhs _ x) -> x) r
                                                   e' <- completeCase insideLet e
