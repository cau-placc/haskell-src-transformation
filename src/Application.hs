module Application (processModule
                    , specialCons
                    , useAlgoModule) where                                      -- TODO too many variables generated
...
useAlgoDecl (FunBind _ ms ) = do nms <- useAlgoMatches ms
                                 return (FunBind () nms)
...
useAlgoMatches (m:ms) = do
    ...
    if (length oneFun) >= 1 || hasCons (m:oneFun) then do
      x  <- useAlgo (m:oneFun)
      xs <- useAlgoMatches r
      return (x:xs)
    else do
...
useAlgo :: [Match ()]    -- all matches for one function name
...
useAlgo ms  = do
    ...
    b    <- gets opt
    if b then do oExp <- optimize nExp
...
specialCons =  [( "unit",      [(Special () (UnitCon ()),0,False)])
               ,( "list",      [(Special () (ListCon ()),0,False),((Special () (Cons ())),2,True)])
               ,( "fun" ,      [(Special () (FunCon ()),2,True)])
               ,( "pair",      [(Special () (TupleCon () Boxed 2),2,False)])    -- TODO Tuples
